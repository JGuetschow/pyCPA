...
def check_consistency(input_DF, tests, columns, folder_test: str = '', data_filter: dict = {}, 
    ...
    all_columns = filtered_DF.columns.values
    ...
    for iRow in range(0, len(table_tests)):
        ...
        if not combo:
            ...
            combined_data = combine_rows(filtered_DF, combo, {}, inplace = False, onlynew = True, verbose = verbose)
            ...
            if combined_data is not None:
                ...
                for value in values_table:
                    ...
                    if combined_data_current is not None:
                        ...
                        for current_column in all_columns_to_check:
                            values_this_col = combined_data.get_unique_meta(current_column)
                        ...
                        if not group_cols_comb == group_cols_ex:
                            ...
                            + 'Comparing not implement for this case. '
                        ...
                        # now we know that the same columns have uniqueb and non-unique values for the 
                        ...
                        data_diff = existing_data.substract(combined_data, {})
                        ...
                        if failed.any():
                            ...
                            for iRow in range(0, len(failed_rows_failed)):
                                ...
                                message = ' ,'.join(failed_rows_deviation.index[iRow]) + ':' 
                                + ' ,'.join(available_years_str[failed_rows_failed]) 
                                + ' ,'.join(failed_rows_deviation[available_years[failed_rows_failed]]) 
                                if verbose:
                                    print(message)
                                if logging:
                                    log.append(message)
                        ...
                        # compare row by row. 
#                        for unique_CC in uniqe_CC_ex:
#                            # get the data
#                            data_comb = combined_data_current.filter(keep = True, inplace = False, 
#                                                                     **dict(zip(columns_compare, unique_CC)))
#                            data_ex = existing_data.filter(keep = True, inplace = False, 
#                                                           **dict(zip(columns_compare, unique_CC)))
#                            # check if units and unit_context are the same
#                            raw_data_ex = data_ex.values
#                            if data_comb.meta['unit'] == data_ex.meta['unit']:
#                                if data_comb.meta['unit_context'] == data_ex.meta['unit_context']:
#                                    # the easiest case, no conversion needed
#                                    raw_data_comb = data_comb.values
#                                else:
#                                    # convert between contexts (e.G. GWPs)
                                
      
                    else: #combined_data_current is None
                        # output and save information
                        if verbose:
                            print(message)
                        if logging:
